...
class SourceCode extends BaseHTMLElement {
    ...
    connectedCallback() {
        ...
        this.$$('a').forEach((link, index) => link.addEventListener('click', (event) => {
...
class WebBrowser extends BaseHTMLElement {
    ...
    async refreshFrame() {
        ...
        const response = await fetch(`http://localhost:3000/api/ping?url=${url}`, { method: 'HEAD' });
...
class WebTerm extends BaseHTMLElement {
    ...
    addTab() {
        ...
        window.querySelector('.newtab').addEventListener('click', (event) => {
            event.preventDefault();
            this.addTab()
        });
    }
